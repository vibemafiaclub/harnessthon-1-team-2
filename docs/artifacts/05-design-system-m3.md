...
폰트는 **②가 자산에서 실측·조달한 값**을 쓴다(고정값이 아니다 — R4 절차 참조).
> 이 프로젝트(당근증권)의 **결과값**: 한글 `Pretendard` · 라틴/숫자 `Inter` · 브랜드 헤드라인 `Jua`
> (토스여행에서는 같은 절차로 자산 실측값이 `Spoqa Han Sans Neo` 로 나왔다 — **값이 아니라 절차가 규칙**이다)
